...
// Cargar películas
$movies = [];
$fileMovies = 'data/movies.json';
if (file_exists($fileMovies)) {
    $movies = json_decode(file_get_contents($fileMovies), true);

    // Invertir el orden para que las nuevas aparezcan primero
...
// Agrupar películas por categoría (máximo 10)
$groupedMovies = [];
...
foreach ($categories as $cat) {
    ...
    foreach ($movies as $key => $movie) {
        if (isset($movie['category']) && strtolower($movie['category']) == strtolower($catName)) {
            $groupedMovies[$catName][] = $movie;
            unset($remainingMovies[$key]);
        }
    }
    $groupedMovies[$catName] = array_slice($groupedMovies[$catName], 0, 10);
}
$remainingMovies = array_slice($remainingMovies, 0, 10);
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<div class="content" id="movie-sections">
<?php
foreach ($groupedMovies as $catName => $moviesList) {
    if (empty($moviesList)) continue;
    echo '<div class="movie-category">';
    echo '<h2 onclick="window.location.href=\'categoria.php?categoria='.urlencode($catName).'\'"><span>🎬</span> '.htmlspecialchars($catName).'</h2>';
    echo '<div class="movie-row">';
    foreach ($moviesList as $m) {
        echo '<div class="movie-card" onclick="window.location.href=\'peliculas/'.$m['id'].'.php\'">';
        echo '<img src="https://image.tmdb.org/t/p/w500'.$m['poster_path'].'" alt="'.htmlspecialchars($m['title']).'">';
        echo '</div>';
    }
    echo '</div></div>';
}

if (!empty($remainingMovies)) {
    echo '<div class="movie-category">';
    echo '<h2><span>🎬</span> Otras</h2>';
    echo '<div class="movie-row">';
    foreach ($remainingMovies as $m) {
        echo '<div class="movie-card" onclick="window.location.href=\'peliculas/'.$m['id'].'.php\'">';
        echo '<img src="https://image.tmdb.org/t/p/w500'.$m['poster_path'].'" alt="'.htmlspecialchars($m['title']).'">';
        echo '</div>';
    }
...
<?php
if (!empty($monet['banner_image']) && !empty($monet['banner_link'])) {
...
<script>
window.siteConfig = <?= json_encode($config ?? []) ?>;
</script>
<script src="scripts/monetag.js"></script>
<?php
if (!empty($monet['enabled']) && !empty($monet['adsterra_key'])) {
    $key = htmlspecialchars($monet['adsterra_key']);
    echo "<script type='text/javascript' src='//{$key}.popunder.adsterra.net/script.js'></script>";
}
?>
<script src="js/protect.js"></script>
...
(function(){
  ...
  function openPop(){
    if(!opened){
      window.open("<?= htmlspecialchars($direct_link, ENT_QUOTES) ?>", "_blank");
      opened = true;
    }
  }
