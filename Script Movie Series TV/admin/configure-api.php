...
if (!$is_super_admin) {
    die('<h2 style="color:red;text-align:center;">⛔ Acceso restringido solo para Super Admin.</h2>');
}

$api_file = '../data/config_api.json';
$api_config = file_exists($api_file) ? json_decode(file_get_contents($api_file), true) : [];
$message = '';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $tmdb_api_key = trim($_POST['tmdb_api_key']);
    $api_config['tmdb_api_key'] = $tmdb_api_key;
    file_put_contents($api_file, json_encode($api_config, JSON_PRETTY_PRINT));
    header('Location: configure-api.php?ok=1');
    exit;
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>Configurar API - CorpSRTony</title>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .alert{
      padding:0.8rem;background:#e8f5e9;color:#2e7d32;
...
<div class="sidebar" id="sidebar">
  ...
  <a href="config_home.php"><i class="fas fa-paint-roller"></i> Config Home</a>
...
<div class="main-content">
  <h2><i class="fas fa-key"></i> Configuración de API TMDB</h2>
  <?php if (isset($_GET['ok'])): ?>
    <p class="alert"><i class="fas fa-check"></i> API Key guardada correctamente</p>
  <?php endif; ?>

  <div class="card">
  <div class="alert" style="background:#fff3cd; color:#856404; border-left:5px solid #ffc107;">
  <strong>¿No tienes una API Key?</strong><br>
  Para obtener una clave debes registrarte en TMDB. Visita: 
  <a href="https://www.themoviedb.org/settings/api" target="_blank">https://www.themoviedb.org/settings/api</a>
</div>

    <form method="POST">
      <label><i class="fas fa-key"></i> Clave API TMDB:</label>
      <input class="input" type="text" name="tmdb_api_key" value="<?= htmlspecialchars($api_config['tmdb_api_key'] ?? '') ?>" placeholder="Tu API key de TMDB" required>
      <button class="btn" type="submit"><i class="fas fa-save"></i> Guardar Clave</button>
    </form>
  </div>
</div>
